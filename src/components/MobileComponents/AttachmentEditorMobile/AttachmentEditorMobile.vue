<template>
    <div id="editor">
        <div style="flex:5;min-height:100px;overflow-y: scroll; max-height: -webkit-fill-available;" class="container3">
            <AttachmentRow v-for="(file,index) in files" :key="index" :file="file" :index="index" @removed="onAttachmentRemoved($event)"/>
        </div>
...
import AttachmentRow from './AttachmentRow/AttachmentRow.vue';
@Component({
    components: {AttachmentRow}

})
export default class AttachmentEditor extends Vue{
    files: File[] = [];
    onFileChanged(e: any){
